...
def Run_Meshroom_Init(overrides, srcImageDir, output, work, pipeline, runStep, version, scale):
	#" --overrides " + overrides + --pxFocals 1458.1
	cmdLine = "C:\\Server\Meshroom\\aliceVision\\bin\\aliceVision_utils_keyframeSelection.exe --mediaPaths " + srcImageDir + " --sensorDbPath C:\\Server\\Meshroom\\aliceVision\\share\\aliceVision\\cameraSensors.db --outputFolder " + srcImageDir + "\\key  --voctreePath C:\\Server\\Meshroom\\aliceVision\\share\\aliceVision\\vlfeat_K80L3.SIFT.tree --useSharpnessSelection 1  --useSparseDistanceSelection 1 --minFrameStep 1 --maxFrameStep 3  --sharpnessPreset very_low  --sharpSubset 1 --sparseDistMaxScore 200 "
	print(cmdLine)
	os.system(cmdLine)
...
def RecServer(srcImageDir):
	...
	while imgcounter < totalImages:
		...
		try:
			print ('RECIEVING IMG')
			#dataIM = b''
			fileName = srcImageDir + "\\IMG"+str(imgcounter)+".jpg"
			#print ('Opened File')
...
def main():
	...
	srcImageDir = "F:\\Viv\Documents\\Uni\\sem1_2019\\FYP\MESHROOM_LIVESCAN\\Images\\" + version
	...
	Run_Meshroom_Init(overrides, srcImageDir, output, work, pipeline, runStep, version, scale)
