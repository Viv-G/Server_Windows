...
def Run_00_CameraInit(baseDir,binDir,srcImageDir, Fov, intMat):
	...
	cmdLine = binName
	#cmdLine = cmdLine + " --defaultFieldOfView " + Fov + " --defaultIntrinsic " + intMat + " --verboseLevel info --sensorDatabase \"\" --allowSingleView 1"
	cmdLine = cmdLine + " --defaultIntrinsic " + intMat + " --verboseLevel info --sensorDatabase \"\" --allowSingleView 1"
...
def Run_01_FeatureExtraction(baseDir,binDir):
	...
	cmdLine = binName
	cmdLine = cmdLine + " --describerTypes sift --forceCpuExtraction True --verboseLevel info --describerPreset high"
...
def Run_07_DepthMap(baseDir,binDir,numImages,groupSize):
	...
	cmdLine = binName
	cmdLine = cmdLine + " --sgmGammaC 5.5 --sgmWSH 4 --refineGammaP 8.0 --refineSigma 15 --refineNSamplesHalf 150 --sgmMaxTCams 10 --refineWSH 3 --downscale " + dScale + " --refineMaxTCams 6 --verboseLevel info --refineGammaC 15.5 --sgmGammaP 8.0"
...
def main():
	...
	print (len(sys.argv))
	if (len(sys.argv) != 6):
		print("usage: python run_alicevision.py <baseDir> <imgDir> <binDir> <runStep> <downscale>")
	...
	print("Downscale    : %s" % dScale)
	print("FOV          : %s" % Fov)
	print("Focal Length : %s" % f)
	print("PPX          : %s" % ppx)
	print("PPY          : %s" % ppy)
	print("Intrinsic Mat: %s" % intMat)
	...
	if runStep == "runall":
		Run_00_CameraInit(baseDir,binDir,srcImageDir, Fov, intMat)
		...
		Run_00_CameraInit(baseDir,binDir,srcImageDir, Fov, intMat)
		Run_01_FeatureExtraction(baseDir,binDir)
		Run_02_ImageMatching(baseDir,binDir)
		Run_03_FeatureMatching(baseDir,binDir)
		Run_04_StructureFromMotion(baseDir,binDir)
